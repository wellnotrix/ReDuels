package dev.veltrix.duels.listeners;
...
import dev.veltrix.duels.util.EventUtil;
...
public class TeamDamageListener implements Listener {
    ...
    public void onTeamMinecartDirectDamage(final EntityDamageByEntityEvent event) {
        ...
        java.util.UUID ownerUuid = ExplosionOwnershipListener.getTntMinecartOwner(minecart);
        if (ownerUuid == null && ExplosionOwnershipListener.isRecentMinecartExplosionInWorld(damaged.getWorld(), 2000)) {
    ...
    public void onTeamBlockExplosion(final EntityDamageEvent event) {
        ...
        if (!ExplosionOwnershipListener.isRecentBlockExplosionInWorld(player.getWorld(), 1500)) {
            return;
        }

        final java.util.UUID trigger = ExplosionOwnershipListener.getRecentBlockExplosionTrigger(player.getWorld());
        if (trigger == null) {
            return;
        }
    ...
    public void onTeamEntityExplosion(final EntityDamageEvent event) {
        ...
        if (!ExplosionOwnershipListener.isRecentMinecartExplosionInWorld(damaged.getWorld(), 2000)) {
            return;
        }
        final java.util.UUID ownerUuid = ExplosionOwnershipListener.getRecentMinecartExplosionOwner(damaged.getWorld());
        if (ownerUuid == null) {
            return;
        }
        final Player owner = damaged.getServer().getPlayer(ownerUuid);
